...
function advanceButton(prevOrNext)
{
    ...
    main( globalURIParamsDict['cur-path'] + '/graph' + globalURIParamsDict.cursor + '.json' );
}
...
function graphTimeChange_obsolete()
{
    ...
    setURI(globalURIParamsDict.cursor);
    main( curPath + '/graph' + globalURIParamsDict.cursor + '.json' );
}
...
function populateGraphSet(endPoint)
{
    ...
    d3.json('/static/graphs' + endPoint + globalURIParamsDict['cur-path'] + '/' + 'menu.json', function(error, menu)
    {
        ...
        for(var i = 0; i<sortedGraphNames.length; i++)
        {
            ...
            option.value = sortedGraphNames[i];
            ...
            localTime.weekdayName + ', ' + localTime.weekday + ')';
...
function addPageDetailsForEndpoint(endPoint)
{
    ...
    document.getElementsByTagName('title')[0].innerHTML = endPoint;
}
...
function processNonURIRequest()
{
    ...
    d3.json('/graphs/pointers' + endPoint, function(error, graphDetails)
    {
        ...
        var storyGraphFilename = globalURIParamsDict['cur-path'] + '/graph' + globalURIParamsDict.cursor + '.json';
...
{   
    ...
    for(var i=0; i<rowCount; i++)
    {
        ...
        for(var j=0; j<colCount; j++)
        {
            ...
            visContainer.className = 'visContainer';
            visContainer.id = visContainer.className + '-' + i + '-' + j;
            td.appendChild(visContainer);

            tdArray[tdArray.length - 1].push(td);
        }
    }

    dynamicTable([], tdArray, '', 'visMatrix');
}
...
{   
    ...
    globalConfig['stopwatch-paused-at'] = {'refresh-seconds': 120, 'max-refresh-seconds': 120};
    globalConfig.focus = 'visContainer-0-0';
    //settings - end
    ...
    if( URIParamsDict.cursor && URIParamsDict.hist && URIParamsDict.t )
    {
        ...
        //populate grid - start
        globalConfig.focus = 'visContainer-0-1';
        main(storyGraphFilename, URIParamsDict.t);

        globalConfig.focus = 'visContainer-1-0';
        main(storyGraphFilename, URIParamsDict.t);

        globalConfig.focus = 'visContainer-1-1';
        main(storyGraphFilename, URIParamsDict.t);
        //populate grid - end
    }
    else
...
function main(storyGraphFilename, timestamp, optionalGraph)
{
    ...
    var visContainer = document.getElementById(globalConfig.focus);
    ...
    d3.select('#' + globalConfig.focus + ' svg').remove();
    var svg = d3.select('#' + globalConfig.focus).append("svg")
        .attr('width', '100%')
        .attr('height', h);
    ...
    var drawGraph = function (graph) 
    {
        ...
        */       
        ...
        node.on("click", function(d)
        {
            ...
            {
                d3.select('#' + globalConfig.focus ).classed("fixed", d.fixed = false); 
            }
        });
...
{
    d3.select('#' + globalConfig.focus).classed("fixed", d.fixed = true);   
}
